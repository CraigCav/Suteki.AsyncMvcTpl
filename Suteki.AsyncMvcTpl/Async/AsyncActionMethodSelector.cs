...
namespace Microsoft.Web.Mvc
{
    ...
    internal sealed class AsyncActionMethodSelector
    {
        ...
        private AmbiguousMatchException CreateAmbiguousActionMatchException(IEnumerable<MethodInfo> ambiguousMethods, string actionName)
        {
            ...
            string message = String.Format(CultureInfo.CurrentUICulture, "The current request for action '{0}' on controller type '{1}' is ambiguous between the following action methods:{2}",
        ...
        {
            string ambiguityList = CreateAmbiguousMatchList(ambiguousMethods);
            string message = String.Format(CultureInfo.CurrentUICulture, "Lookup for method '{0}' on controller type '{1}' failed because of an ambiguity between the following methods:{2}",
        ...
        private static string CreateAmbiguousMatchList(IEnumerable<MethodInfo> ambiguousMethods)
        {
            ...
            foreach (MethodInfo methodInfo in ambiguousMethods)
            {
                ...
                exceptionMessageBuilder.AppendFormat(CultureInfo.CurrentUICulture, "{0} on type {1}", methodInfo, methodInfo.DeclaringType.FullName);
            }
        ...
        private ActionDescriptorCreator GetActionDescriptorDelegate(MethodInfo entryMethod)
        {
            ...
            if (entryMethod.Name.StartsWith("Begin", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (endMethod == null)
                {
                    string errorMessage = String.Format(CultureInfo.CurrentUICulture, "Could not locate a method named '{0}' on controller type '{1}'.", endMethodName, ControllerType.FullName);
            ...
            {
                return (actionName, controllerDescriptor) => new ReflectedTaskPatternActionDescriptor(entryMethod, controllerDescriptor);
            }
